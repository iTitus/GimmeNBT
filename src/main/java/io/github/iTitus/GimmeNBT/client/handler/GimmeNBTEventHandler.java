	...
	public void onItemToolTipAdded(ItemTooltipEvent event) {
		if (event.showAdvancedItemTooltips
			...
			event.toolTip.add(StatCollector.translateToLocalFormatted(
					"tooltip.gimmenbt.id", Item.itemRegistry
							.getNameForObject(event.itemStack.getItem()), Item
							.getIdFromItem(event.itemStack.getItem())));

			// Name
			event.toolTip.add(StatCollector.translateToLocalFormatted(
					"tooltip.gimmenbt.unlocalizedName",
					event.itemStack.getUnlocalizedName()));

			// Damage
			int types = Utils.getMaxDamage(event.itemStack);
			if (types > 0)
				event.toolTip.add(StatCollector.translateToLocalFormatted(
						"tooltip.gimmenbt.damage",
						event.itemStack.getItemDamage(), types));

			// Max stack size
			event.toolTip.add(StatCollector.translateToLocalFormatted(
					"tooltip.gimmenbt.maxStackSize",
					event.itemStack.getMaxStackSize()));

			// NBT
			NBTTagCompound nbt = event.itemStack.getTagCompound();
			if (nbt != null) {
				event.toolTip.add(StatCollector
