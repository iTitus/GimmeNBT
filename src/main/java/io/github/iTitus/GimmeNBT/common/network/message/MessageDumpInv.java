...
public class MessageDumpInv implements IMessage, IMessageHandler<MessageDumpInv, IMessage> {
    ...
    public MessageDumpInv(IInventory inv, boolean isBlockInv, String inventoryOwner) {
        if (inv != null) {
    ...
    public IMessage onMessage(MessageDumpInv msg, MessageContext ctx) {
        FMLClientHandler.instance().getClientPlayerEntity().addChatComponentMessage(Utils.makeInvDump(msg.name, msg.originName, msg.hasCustomName, msg.slotNumber, msg.stackLimit, msg.items, msg.isBlockInv, msg.inventoryOwner));
